function [mean_duty_cycle] = generate_plots_for(freq_axis, all_time_offsetted, all_power, start_freq, stop_freq, start_period, stop_period, filepath, margin)
    ...
    plot(f_array, min(spectrum_array), 'g', f_array, mean(spectrum_array), 'b', f_array, max(spectrum_array), 'r', f_array, noise_mean, 'm', f_array, threshold, 'k')
    ...
    mean_duty_cycle = mean((sum(duty_cycle)/max(sum(duty_cycle)) * 100));
